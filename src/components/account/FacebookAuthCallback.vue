...
onMounted(async () => {
  ...
  if (accessToken && refreshToken) {
    try {
      await authService.handleFacebookCallback(accessToken, refreshToken);
      showSuccess(toast, "Đăng nhập Facebook thành công");
      await router.push('/');
      setTimeout(() => {
        window.location.reload();
      }, 500);
    } catch (error) {
      console.error('Lỗi xử lý đăng nhập Facebook:', error);
      if (error.response) {
        // Xử lý lỗi từ server
        ...
        // Xử lý lỗi khác
        showError(toast, "Lỗi xử lý đăng nhập Facebook");
      }
